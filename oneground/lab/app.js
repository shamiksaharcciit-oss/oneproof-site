...
const DATA_VERSION = {"base.bin": "08268d20", "centroids.json": "26cd059b", "inline.js": "54fb63d4", "queries.json": "97289c3a", "values.json": "81be970b"};
// --- end generated ---
...
function buildGround(values, D, Q, centroids, copies, catColours, cats) {
  ...
  function caption(s) {
    ...
    // 1(b), core's wording (task 044e/044i). measured.k_sweep — read, not
    // stated. The sweep's own row at this page's count must reproduce the
    // published figure exactly; if it does not, the two numbers are not the
    // same measurement and the sentence does not render.
    //
    // It states the crisp share itself rather than the complement the
    // sentence before it shows: that one renders as a whole percentage
    // ("96%"), so "96%" beside "0.053" would share neither unit nor
    // direction, and as complements the pair is 96% → 95%, which hides the
    // 46% relative move the sentence exists to show.
    const sw = values.measured.k_sweep;
    const nc = values.geometry.n_centroids;
    const at = (k) => (sw ? sw.rows.find((r) => r.k === k) : undefined);
    const here = at(nc), hi = at(2048);
    const tied = !!(here && hi && here.boundary_crispness === crisp);
    if (s.copied === 0) {
      return 'At ε = 0.00 every vector sits within ε of exactly one region — ' +
        ...
        (tied
          ? ' Cut the same corpus into <b>' + fmtInt(hi.k) + '</b> regions and the share ' +
            'with no second centroid that close rises from <b>' + fmtPct(crisp, 1) +
            '</b> to <b>' + fmtPct(hi.boundary_crispness, 1) + '</b>: the count is part ' +
            'of the reading.'
          : '');
    }
    if (p4 >= 0.5) {
